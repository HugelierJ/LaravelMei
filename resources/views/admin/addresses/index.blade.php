    ...
    <table class="table table-striped shadow-lg p-3 mb-5 bg-body-tertiary rounded">
        ...
        @foreach($addresses as $address)
            <tr>
                ...
                <td>{{$address->city}}</td>
                <td>{{$address->state}}</td>
                <td>{{ $address->zip_code ?? "Not Available" }}</td>
                ...
                <td>
                    <div class="nav-item dropdown no-arrow">
                        <a class="nav-link dropdown-toggle" href="#" id="userDropdown{{ $address->id }}" role="button"
                        ...
                        <div class="dropdown-menu  shadow "
                             aria-labelledby="userDropdown{{ $address->id }}">
                            <a class="dropdown-item" href="{{route('products.edit', $address->id)}}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                            ...
                            <form action="{{ route('products.destroy', $address->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
